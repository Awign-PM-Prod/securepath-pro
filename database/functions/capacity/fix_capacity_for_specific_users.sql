-- =====================================================
-- Fix Capacity Data for Specific Users
-- =====================================================

-- This script fixes capacity data for users who show consumed capacity
-- but have no actual case assignments

-- Step 1: Check current state
SELECT 
  'BEFORE FIX - Current State' as info,
  p.email,
  p.first_name,
  p.last_name,
  gp.max_daily_capacity,
  gp.capacity_available,
  gp.active_cases_count,
  (SELECT COUNT(*) FROM public.cases c WHERE c.current_assignee_id = gp.id) as actual_cases
FROM public.profiles p
JOIN public.gig_partners gp ON p.id = gp.profile_id
WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com');

-- Step 2: Fix gig_partners table capacity data
UPDATE public.gig_partners 
SET 
  capacity_available = max_daily_capacity,
  active_cases_count = 0,
  updated_at = now()
WHERE id IN (
  SELECT gp.id 
  FROM public.profiles p
  JOIN public.gig_partners gp ON p.id = gp.profile_id
  WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com')
);

-- Step 3: Fix capacity_tracking table for today
UPDATE public.capacity_tracking 
SET 
  current_capacity_available = max_daily_capacity,
  cases_allocated = 0,
  cases_accepted = 0,
  cases_in_progress = 0,
  cases_submitted = 0,
  cases_completed = 0,
  updated_at = now()
WHERE gig_partner_id IN (
  SELECT gp.id 
  FROM public.profiles p
  JOIN public.gig_partners gp ON p.id = gp.profile_id
  WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com')
)
AND date = CURRENT_DATE;

-- Step 4: If no capacity_tracking record exists for today, create one
INSERT INTO public.capacity_tracking (
  gig_partner_id,
  date,
  max_daily_capacity,
  initial_capacity_available,
  current_capacity_available,
  cases_allocated,
  cases_accepted,
  cases_in_progress,
  cases_submitted,
  cases_completed,
  last_reset_at,
  is_active,
  created_at,
  updated_at
)
SELECT 
  gp.id,
  CURRENT_DATE,
  gp.max_daily_capacity,
  gp.max_daily_capacity,
  gp.max_daily_capacity,
  0,
  0,
  0,
  0,
  0,
  now(),
  true,
  now(),
  now()
FROM public.profiles p
JOIN public.gig_partners gp ON p.id = gp.profile_id
WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com')
AND NOT EXISTS (
  SELECT 1 FROM public.capacity_tracking ct 
  WHERE ct.gig_partner_id = gp.id 
  AND ct.date = CURRENT_DATE
);

-- Step 5: Verify the fix
SELECT 
  'AFTER FIX - Updated State' as info,
  p.email,
  p.first_name,
  p.last_name,
  gp.max_daily_capacity,
  gp.capacity_available,
  gp.active_cases_count,
  (SELECT COUNT(*) FROM public.cases c WHERE c.current_assignee_id = gp.id) as actual_cases
FROM public.profiles p
JOIN public.gig_partners gp ON p.id = gp.profile_id
WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com');

-- Step 6: Check capacity tracking after fix
SELECT 
  'Capacity Tracking After Fix' as info,
  p.email,
  ct.date,
  ct.max_daily_capacity,
  ct.current_capacity_available,
  ct.cases_allocated
FROM public.profiles p
JOIN public.gig_partners gp ON p.id = gp.profile_id
LEFT JOIN public.capacity_tracking ct ON gp.id = ct.gig_partner_id
WHERE p.email IN ('rajesh.sharma@example.com', 'test.user3@example.com')
AND ct.date = CURRENT_DATE;

-- Step 7: General capacity verification for all users
SELECT 
  'All Users Capacity Check' as info,
  p.email,
  gp.max_daily_capacity,
  gp.capacity_available,
  gp.active_cases_count,
  (SELECT COUNT(*) FROM public.cases c WHERE c.current_assignee_id = gp.id) as actual_cases,
  CASE 
    WHEN gp.capacity_available != (gp.max_daily_capacity - (SELECT COUNT(*) FROM public.cases c WHERE c.current_assignee_id = gp.id))
    THEN 'MISMATCH'
    ELSE 'OK'
  END as status
FROM public.profiles p
JOIN public.gig_partners gp ON p.id = gp.profile_id
WHERE gp.is_active = true
ORDER BY status DESC, p.email;
