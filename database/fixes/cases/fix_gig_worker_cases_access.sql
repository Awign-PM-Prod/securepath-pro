-- Fix gig worker access to cases and create test data
-- This script ensures gig workers can access their allocated cases

-- First, let's check the current case statuses and fix them if needed
DO $$
DECLARE
  case_count integer;
  allocated_count integer;
  auto_allocated_count integer;
BEGIN
  SELECT COUNT(*) INTO case_count FROM public.cases;
  SELECT COUNT(*) INTO allocated_count FROM public.cases WHERE status = 'allocated';
  SELECT COUNT(*) INTO auto_allocated_count FROM public.cases WHERE status = 'auto_allocated';
  
  RAISE NOTICE 'Total cases: %', case_count;
  RAISE NOTICE 'Cases with status "allocated": %', allocated_count;
  RAISE NOTICE 'Cases with status "auto_allocated": %', auto_allocated_count;
  
  -- Update cases with 'allocated' status to 'auto_allocated' for consistency
  IF allocated_count > 0 THEN
    UPDATE public.cases 
    SET status = 'auto_allocated' 
    WHERE status = 'allocated';
    RAISE NOTICE 'Updated % cases from "allocated" to "auto_allocated"', allocated_count;
  END IF;
END $$;
...
USING (
  ...
  (current_assignee_type = 'gig_worker' AND current_assignee_id IN (
  ...
  (has_role('vendor') AND current_assignee_type = 'gig_worker' AND current_assignee_id IN (
...
BEGIN
  ...
  IF test_gig_worker_id IS NOT NULL THEN
    ...
    IF case_count > 0 THEN
      ...
      FOR rec IN 
        SELECT case_number, title, status, priority, base_rate_inr
        FROM public.cases 
        WHERE current_assignee_id = test_gig_worker_id
        LIMIT 3
      LOOP
        RAISE NOTICE '  - %: % (%) - % - ₹%', 
          rec.case_number, rec.title, rec.status, rec.priority, rec.base_rate_inr;
      END LOOP;
    END IF;
  END IF;
END $$;
