-- Update contract_type enum values and add unique constraint (FIXED VERSION)
...
BEGIN
    ...
    FROM (
        SELECT client_id, contract_type, COUNT(*) as cnt
    ...
    IF duplicate_count > 0 THEN
        RAISE NOTICE 'Found % duplicate client_id, contract_type combinations', duplicate_count;
        
        -- Show duplicates for debugging
        FOR client_id_val, contract_type_val IN
        ...
        LOOP
            RAISE NOTICE 'Duplicate: client_id=%, contract_type=%', client_id_val, contract_type_val;
        END LOOP;
        
        -- Keep only the most recent record for each duplicate combination
        ...
        RAISE NOTICE 'Removed duplicate records, keeping only the most recent ones';
    ELSE
