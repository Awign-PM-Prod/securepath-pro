...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view capacity tracking they are authorized to see" ON public.capacity_tracking;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view allocation logs they are authorized to see" ON public.allocation_logs;
DROP POLICY IF EXISTS "Users can create allocation logs they are authorized to manage" ON public.allocation_logs;
DROP POLICY IF EXISTS "Users can update allocation logs they are authorized to manage" ON public.allocation_logs;
...
-- Test the policies by checking if ops_team can create gig_worker profiles
SELECT 
