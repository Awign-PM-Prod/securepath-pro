-- =====================================================
-- Fix Vendor Constraint Logic
-- Background Verification Platform
-- =====================================================

-- The current constraint is incorrect. It should be:
-- When current_assignee_type = 'gig', current_vendor_id should be the vendor who owns that gig worker
-- When current_assignee_type = 'vendor', current_vendor_id should equal current_assignee_id
...
CHECK (
  ...
  -- When assignee type is 'gig', vendor_id should be the vendor who owns the gig worker
  (
    (current_assignee_type = 'gig'::assignment_type) AND 
    (current_vendor_id IS NOT NULL) AND 
    (current_vendor_id IN (
      SELECT vendor_id FROM gig_partners WHERE id = current_assignee_id
    ))
  ) OR 
...
-- Update the assignment function to work with the corrected constraint
