-- =====================================================
-- Update Create User Edge Function for Vendor Support
-- Background Verification Platform
-- =====================================================

-- This script creates a database function that mimics the create-user Edge Function behavior
-- Run this in Supabase SQL Editor to create the function

-- Drop existing function if it exists
DROP FUNCTION IF EXISTS public.create_user_edge_function;

CREATE OR REPLACE FUNCTION public.create_user_edge_function(
  user_email TEXT,
  user_password TEXT,
...
GRANT EXECUTE ON FUNCTION public.create_user_edge_function TO authenticated;

-- Test the function with vendor creation
SELECT public.create_user_edge_function(
  'test@vendor.com',
  'password123',
  'Test',
  'Vendor',
...
-- Test the function with gig worker creation (using a real vendor ID if available)
-- First, let's get a real vendor ID
DO $$
DECLARE
  test_vendor_id UUID;
BEGIN
  -- Get the first available vendor ID
  SELECT id INTO test_vendor_id FROM public.vendors LIMIT 1;
  
  IF test_vendor_id IS NOT NULL THEN
    -- Test gig worker creation with real vendor ID
    PERFORM public.create_user_edge_function(
      'test@gigworker.com',
      'password123',
      'Test',
      'Gig Worker',
      '+91 98765 43211',
      'gig_worker',
      jsonb_build_object('vendor_id', test_vendor_id)
    );
    RAISE NOTICE 'Gig worker test completed with vendor ID: %', test_vendor_id;
  ELSE
    RAISE NOTICE 'No vendors found, skipping gig worker test';
  END IF;
END $$;
