...
-- First, drop the existing functions that need to be recreated
DO $$ 
BEGIN
    -- Drop functions that will be recreated with new signatures
    -- Only drop if they exist to avoid errors
    
    -- Drop get_case_defaults if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p 
               JOIN pg_namespace n ON p.pronamespace = n.oid 
               WHERE p.proname = 'get_case_defaults' 
               AND n.nspname = 'public'
               AND pg_get_function_identity_arguments(p.oid) = 'uuid, text, integer') THEN
        DROP FUNCTION public.get_case_defaults(uuid, text, integer);
    END IF;
    
    -- Drop get_rate_card_for_client_tier if it exists (with any signature)
    IF EXISTS (SELECT 1 FROM pg_proc p 
               JOIN pg_namespace n ON p.pronamespace = n.oid 
               WHERE p.proname = 'get_rate_card_for_client_tier' 
               AND n.nspname = 'public') THEN
        -- Drop all variants of this function
    ...
    -- Drop get_location_from_pincode if it exists
    IF EXISTS (SELECT 1 FROM pg_proc p 
               JOIN pg_namespace n ON p.pronamespace = n.oid 
               WHERE p.proname = 'get_location_from_pincode' 
               AND n.nspname = 'public'
               AND pg_get_function_identity_arguments(p.oid) = 'text') THEN
        DROP FUNCTION public.get_location_from_pincode(text);
    END IF;
END $$;
